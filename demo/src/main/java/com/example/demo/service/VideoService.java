...
public class VideoService {
    ...
    public String testService() {
        // TodoEntity 생성
        VideoEntity entity = VideoEntity.builder().title("My first Video item").build();
        // TodoEntity 저장
        repository.save(entity);
        // TodoEntity 검색
        VideoEntity savedEntity = repository.findById(entity.getId()).get();
        return savedEntity.getTitle();
    }

    public List<VideoEntity> create(final VideoEntity entity) {
        // Validations
        validate(entity);
    ...
    private void validate(final VideoEntity entity) {
        if(entity == null) {
    ...
    public List<VideoEntity> retrieve(final String userId) {
    ...
    public List<VideoEntity> update(final VideoEntity entity) {
        ...
        final Optional<VideoEntity> original = repository.findById(entity.getId());



        original.ifPresent(video -> {
            // (3) 반환된 TodoEntity가 존재하면 값을 새 entity의 값으로 덮어 씌운다.
            video.setTitle(entity.getTitle());
            //video.setDone(entity.isDone());
    ...
    public List<VideoEntity> delete(final VideoEntity entity) {
        // (1) 저장 할 엔티티가 유효한지 확인한다. 이 메서드는 2.3.1 Create Todo에서 구현했다.
        validate(entity);
